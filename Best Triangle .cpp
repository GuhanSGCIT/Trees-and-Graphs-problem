...
Explanation
There are 4 triangles in total, and their areas are: 1, 1, 2 and 4. The ones which does not exceed 7/2 are 1, 1 and 2 so the biggest area is 2, and since we are required to multiply the area by 2 before outputting it, it becomes 4.
...
hint:
We iterate over the slopes in sorted order w.r.t their slope. We maintain the list LLL of points sorted w.r.t their height from the slope. For each slope, we do two binary searches to find the maximum area triangle with area less than or equal to K/2, and also update the list L by making one swap.
